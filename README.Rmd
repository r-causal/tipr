...
We are interested in a continuous unmeasured confounder, so we will use the `tip_with_continuous()` function. The package comes with an example data set simulated for this scenario, `exdata_continuous`. 

Let's assume the relationship between the unmeasured confounder and outcome is 1.5 (`confounder_outcome_effect = 1.5`),  let's solve for the relationship between the exposure and unmeasured confounder needed to tip the analysis (in this case, we are solving for `exposure_confounder_effect`, the mean difference needed between the exposed and unexposed).
