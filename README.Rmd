...
exdata_rr %>%
  group_by(exposure) %>%
  summarise(m = mean(.unmeasured_confounder / sd(.unmeasured_confounder))) %>%
  pivot_wider(names_from = exposure,
              values_from = m,
              names_prefix = "u_") %>%
  summarise(estimate = u_1 - u_0)
```
...
It would take about `7` independent standardized Normal unmeasured confounders with a mean difference between exposure groups of 0.25 to and a relationship with the outcome of 1.05 tip the observed analysis at the 5% level, rendering it inconclusive.
