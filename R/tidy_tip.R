...
#'            gamma = 1.1)
#'}
tidy_tip <- function(mod, exposure, p0 = NULL, p1 = NULL, gamma = NULL) {
  ...
  if (!is.null(gamma) & !is.null(p0) & !is.null(p1)) {
    n <- tip_n(p0, p1, gamma, b, lb, ub)
  }

  if (is.null(gamma)) {
    gamma <- tip_gamma(p0, p1, b, lb, ub)
  }

  if (is.null(p0)){
    p0 <- tip_p0(p1, gamma, b, lb, ub)
  }

  if (is.null(p1)){
    p1 <- tip_p1(p0, gamma, b, lb, ub)
  }
  tibble::tibble(
    lb = lb,
    ub = ub,
